...
class tushareDataDB(SqlHandle):
    def __init__(self):
        self.db=pymysql.connect("localhost","root","123123","tushare_stock_data",charset='utf8')
        self.cursor=self.db.cursor()      
    def readDatas(self,code,*args):
        if len(args)==0:
            sql="SELECT * FROM tushare_stock_data."+code+";"
        elif len(args)==1:
            sql="SELECT * FROM tushare_stock_data."+code+" where date >= '"+args[0]+"';"
        elif len(args)==2:
            sql="SELECT * FROM tushare_stock_data."+code+" where date >= '"+args[0]+"' and date<'"+args[1]+"';"
        else:
            print('wrong input in getdays()!')
            return None
        return self.fetchall(sql)
    def readData(self,code,date):
        sql="SELECT * FROM tushare_stock_data."+code+" where date = '"+date+"';"
        return self.fetchall(sql)
